...
********)
\* BEGIN TRANSLATION (chksum(pcal) = "6a5e63d4" /\ chksum(tla) = "e7c6ba5")
...
Lbl_2 == /\ pc = "Lbl_2"
         /\ IF i =< Len(inp)
               THEN /\ IF inp[i] > max
                    ...
                    /\ i' = i + 1
                    /\ pc' = "Lbl_2"
               ELSE /\ Assert(IF inp = << >> THEN max = -99999
                                 ELSE    (\E n \in 1..Len(inp) : max = inp[n])
                                      /\ (\A n \in 1..Len(inp) : max >= inp[n]), 
...
\* Last modified Wed Jan 10 23:31:16 CET 2024 by alexander
